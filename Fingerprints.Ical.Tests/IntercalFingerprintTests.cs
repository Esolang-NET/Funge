...
{
    static async Task<FingerprintInstruction> Instruction(IntercalFingerprint fp, char ch)
    {
    ...
    public async Task Handprint_IsCorrect()
        => await Assert.That(new IntercalFingerprint().Handprint).IsEqualTo(0x4943414C);

    [Test]
    public async Task Instruction_I_Mingle()
    {
        ...
        await Assert.That(ctx.Pop()).IsEqualTo(0b10011001);
    }

    [Test]
    public async Task Instruction_S_Select()
    {
        var fp = new IntercalFingerprint();
        var ctx = new TestContext();
    ...
    [Test]
    public async Task Instruction_A_UnaryAnd_16bit()
    {
        var fp = new IntercalFingerprint();
        var ctx = new TestContext();
    ...
    [Test]
    public async Task Instruction_N_Next_And_R_Resume()
    {
        var fp = new IntercalFingerprint();
    ...
    [Test]
    public async Task Instruction_F_Forget()
    {
        var fp = new IntercalFingerprint();
    ...
    [Test]
    public async Task Instruction_N_NoPositionContext_Reflects()
    {
        var fp = new IntercalFingerprint();
    ...
    [Test]
    public async Task Instruction_R_NegativeN_Reflects()
    {
        var fp = new IntercalFingerprint();
        var ctx = new TestContext();
        ctx.Push(-1);
